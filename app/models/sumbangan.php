...
class sumbangan extends Model
{
    ...
    public static function isThere($jsumbang)
    {
        ...
        try {
            $db = static::getDB();
            $stmt = $db->query("SELECT `id` FROM `jenis_sumbangan` WHERE `name`='".$jsumbang."' LIMIT 1");
    ...
    public static function setJS($jsumbang)
    {
        ...
        try {
            $db = static::getDB();
            $stmt = $db->query("INSERT INTO `jenis_sumbangan`(`name`) VALUES ('". $jsumbang ."')");
            $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
            return $db->lastinsertId();
    ...
    public static function setSumbangan($userid,$jenisid,$jumlah)
    {
        ...
        try {
            $db = static::getDB();
            $stmt = $db->query("INSERT INTO `sumbangan`(`userid`, `jenis`, `jumlah`) VALUES ('".$userid."' ,'".$jenisid."','".$jumlah."')");
            $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
            return;
